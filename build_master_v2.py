...
FRAMES_DIR = os.path.join(BASE_DIR, "frames_v2")
TEMP_DIR = os.path.join(BASE_DIR, "v2_temp")
os.makedirs(TEMP_DIR, exist_ok=True)

FINAL_OUTPUT = os.path.join(BASE_DIR, "video_microlearning_D_Đ.mp4")
W, H = 1920, 1080
FPS = 30
TRANSITION_FRAMES = 12 # 0.4s smooth crossfade dissolve
...
# Build sequence of (frame_image_filename, list_of_wavs)
timeline = [
    # Scene 1: Intro
...
print("4. Rendering video with smooth crossfade transitions...")
t0 = time.time()
ffmpeg_cmd = [
    "ffmpeg", "-y",
...
for idx in range(num_segments):
    ...
    # Check if there is a next segment to transition to
    if idx < num_segments - 1:
        next_img_path, _ = segment_frames[idx + 1]
        # Only crossfade if the next image is actually different
...
print(f"MASTERPIECE CREATED: {FINAL_OUTPUT}")
